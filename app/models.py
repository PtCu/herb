...
class Camera(models.Model):
    ...
    model_type = models.CharField("型号", max_length=20)  # 型号
    state = models.BooleanField("是否正常运行", default=False, blank=True)  # 是否坏了
    start_time = models.DateTimeField("运行时间",default=datetime.now(), blank=True)  # 开始运行时间
...
class Plant(models.Model):
    ...
    time = models.DateTimeField("种植时间", default=datetime.now())
...
class IncubatorHistory(models.Model):
    curTime = models.DateTimeField("当前记录时间", default=timezone.now())  # 定时存入数据库的时间
